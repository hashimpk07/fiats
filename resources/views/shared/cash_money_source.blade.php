<div class="form-group ">
	...
	<div class="col-sm-9">
		<input value="1" id="radModeAccount" name="radMode" class="radioCheck radioMode1" type="radio" checked="checked"
		       onclick="jQuery('.money-source-group-rad').hide();jQuery('.money-source-account').show();jQuery('#btnSaveClaim').show();jQuery('.no-claim').show();">
		Account &nbsp; &nbsp; &nbsp;
		<input value="2" id="radModeAdvance" name="radMode" class="radioCheck radioMode2" type="radio"
		       onclick="jQuery('.money-source-group-rad').hide();jQuery('.money-source-advance').show();jQuery('#btnSaveClaim').show();jQuery('.no-claim').show();">
		Advance &nbsp; &nbsp; &nbsp;
		<input value="3" id="radModeClaim" name="radMode" class="radioCheck radioMode3" type="radio"
		       onclick="jQuery('.money-source-group-rad').hide();jQuery('.money-source-claim').show();jQuery('#btnSaveClaim').hide();jQuery('.no-claim').hide();">
		Claim
...
<div class="form-group money-source-group-rad money-source-account">
	...
	<div class="col-sm-9">
		<select name="selAccount" id="selAccount"
		        onchange="postField('{{ action('AjaxController@getAccountCurrencyOptions') }}', '#selAccount', {}, null, null, function(data){ jQuery('#selCurrency').html(data);FillCurrency(); } );"
	...
	<div class="col-sm-9">
		<select class="col-xs-10 col-sm-5" name="selClaim" id="selClaim">
			...
			@foreach( $claimOptions as $key => $value )
				<option
					{{ ((@$record->claim_id == $key) ? 'selected="selected' : '') }} value="{{ $key }}">{{ $value }}</option>
			@endforeach
		</select>
